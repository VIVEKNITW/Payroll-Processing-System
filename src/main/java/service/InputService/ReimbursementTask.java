...
public class ReimbursementTask implements Task {
    ...
    public void readInput(String[] parts) throws ParseException {
        ...
        EventType eventType = EventType.REIMBURSEMENT;
        Long reimbVal = Long.parseLong(parts[3].trim());
        ...
        if(employeeList.list().containsKey(empId)){
            ...
            myValue.setReimbursement(reimbVal);
            myEmployee.setValue(myValue);
            employeeList.list().put(empId, myEmployee);
        }

        Event event = new Event.Builder().setEventNo(seqenceNo).setEventDate(eventDate).setEventType(eventType).setNotes(note).setAmount(reimbVal).setEmpId(empId).build();
