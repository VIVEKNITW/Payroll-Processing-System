...
public class BonusTask implements Task {
    ...
    public void readInput(String[] parts) throws ParseException {
        ...
        EventType eventType = EventType.BONUS;
        Long bonus = Long.parseLong(parts[3].trim());
        ...
        if(employeeList.list().containsKey(empId)){
            ...
            myValue.setBonus(bonus);
            myEmployee.setValue(myValue);
            employeeList.list().put(empId, myEmployee);
        }

        Event event = new Event.Builder().setEventNo(seqenceNo).setEventDate(eventDate).setEventType(eventType).setNotes(note).setAmount(bonus).setEmpId(empId).build();
